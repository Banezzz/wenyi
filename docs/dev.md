...
- `glossary/store.py` 拥有 SQLite 术语、冲突、翻译记忆、batch/window 子 checkpoint
  和章级父 checkpoint 的事务与完成判定。
- `pipeline/glossary_windows.py` 负责确定性窗口计划、v1 兼容检查和 v2 章级对账；编排器只
  提供抽取回调与生命周期衔接。
- `pipeline/runstore.py` 持久化 manifest、章节 JSON、上下文和追加式事件日志。
- `assemble/` 只消费已持久化结果并导出 EPUB/TXT，不决定翻译或术语进度。

EPUB 回填把目录译名视为显式数据，而不是按文件名猜测：writer 只使用 manifest 中真实存在的
`title_translated`，以规范化 ZIP 完整路径和原标签共同匹配 NCX/NAV 项。章节级译名最多改写
对应文档中首个匹配目录项；带 fragment 的小节目录没有独立译名时保持原文，不能被父章节
标题批量覆盖。同名但位于不同目录的 XHTML 也不会互相串写。章节与额外 TOC 文档的 HTML
`<title>`、EPUB2 guide 标题和目录语言会同步更新；全书 OPF/NCX 书名按当前产品约定保留原文。
...
独立 `extract()` 调用可逐条跳过 required 字段无效的候选；会写术语与 checkpoint 的
`extract_and_store()` 更严格：只要非空 `terms` 数组中有任一候选被拒绝，整次响应即标为
`terms_rejected`，不写有效子集也不写 checkpoint。这样部分畸形响应不能被误当作完整抽取
证明，下一次续跑仍会重新请求整个窗口。

空抽取结果采用更严格的协议：只有根对象精确为 `{"terms":[]}` 才能证明“本窗口没有新
术语”。缺少 `terms`、类型错误或夹带其他根字段的空结果都属于协议失败，不能写入完成
checkpoint。非空候选仍按上表逐条清洗。

术语抽取不能使用会从混合文本中寻找 JSON 片段的宽松解析器。Extractor 直接取得模型原始
文本，去除首尾空白后对**完整响应**执行一次 `json.loads`，并要求顶层对象只有唯一的
`terms` 键。因此拒答、自然语言解释、Markdown 代码围栏，或在这些文本中嵌入一段看似合法
JSON，都会作为 `model_or_json_error` 失败；顶层额外键则作为 `response_extra_keys` 失败。
这些失败都不写术语或 checkpoint，章节保持 glossary pending，续跑可以重新请求。
...
章级复核不再把整章一次塞进模型。新章节在 canonical unit 之上另存 version 1
`chapter.meta.chapter_glossary_plan`（`chapter_window` 计划）：
...
每个窗口最多包含 3 个 canonical unit，按章内连续段落计算的源文不超过 6000 字符；存在
后续窗口时，下一窗口从前一窗口最后一个 unit 开始，固定重叠 1 个 unit。单个 canonical
unit 已超过上限，或无法在上限内保留重叠时，计划校验失败并停止续跑。窗口身份和整个计划
均有稳定指纹，续跑按已持久化计划恢复，不能根据本轮完成位置重排窗口。

最终发给术语模型的 system/user messages 还必须按真实序列化结果不超过 30000 字符。
源文和译文不能为凑预算被截断；超限时只能逐条移除完整的参考术语项。即使没有任何参考
术语仍超限，也应把该窗口保留为 pending 并记录协议失败，而不是提交不完整 checkpoint。

旧 run 已经写入的直接章级 checkpoint 保持兼容：全部 batch 子记录与
`scope=chapter, version=1` 父记录仍按原来的 source/target 指纹验证。只有带
`chapter_window` 计划的新路径才使用由子 checkpoint 推导的 `scope=chapter, version=2`
父记录。

## SQLite 原子 checkpoint

单个 batch 或 `chapter_window` 子单元的提交顺序为：

1. 在内存中验证全部术语、chapter 参数和 checkpoint 字段。
2. 开启一个 SQLite 事务。
3. 在同一事务中写 glossary、可能产生的 `term_conflicts` 和对应子 checkpoint。
4. 任一步失败则 rollback；成功后一次 commit。

事务边界保证不会出现“术语写了一半但 checkpoint 已完成”或“冲突记录残留但主写入失败”。
batch 和兼容章级记录位于 `glossary_extraction_checkpoints`；窗口子记录位于独立的
`glossary_chapter_window_checkpoints`，以 `(chapter, start_index)` 为主键，并绑定
`plan_fingerprint`。checkpoint 匹配 `version`、`count`、
...
新路径的章级 v2 父 checkpoint 不是独立的 LLM 成功标记。所有预期 batch/window 子记录提交
后，store 在 `BEGIN IMMEDIATE` 事务内读取并核对当前 generation 下的**精确有序子集合**；
缺少、额外、失配或属于另一计划的子记录都会拒绝推导。通过后，以 domain-separated SHA-256
绑定 generation、计划指纹及全部子身份，写入 `scope=chapter, version=2` 父记录并一次提交。
续跑若在最后一个子记录提交后、父记录提交前中断，可以只重新推导父记录，不需要再次请求
已经匹配的窗口。

因此 v2 的 `done` 判定必须同时满足三项：全部预期 batch 子记录精确匹配、全部预期 window
子记录精确匹配、按同一子集合重新推导出的 v2 父记录匹配。只看父记录、只看 manifest，或
只验证子记录存在都不足以证明完成。
...
- `glossary_status=pending|done`：canonical batch plan、章级窗口计划及其派生父记录是否均有
  有效 SQLite checkpoint；兼容旧 run 时由全部 batch 与直接章级 v1 父记录共同证明。
...
`titles_status=done` 只证明译名已经持久化；导出正确性另由 EPUB 回填契约保证。源 EPUB 的
NCX 可能包含同一 XHTML 下的多级 fragment 导航，而旧状态只追踪每个文档的首个 TOC 标签，
因此 writer 必须精确改写已追踪父项并保留未追踪子项的标签与层级。

标题翻译只注入当前章节标题和额外 TOC 标题源文实际命中的术语。编排器先取术语库快照，
再用与正文一致的 source/alias 边界匹配筛选，最后才渲染 glossary reference；未命中的全局
术语不进入标题请求。真实书验证中，参考集合从 10931 项、628233 字符缩减到 56 项、3487
字符，避免整本术语库把最终标题请求推过上下文上限。`titles_translated` 同时记录筛选前后的
术语数，便于后续定位 prompt 规模回归。
...
4. 所有 batch checkpoint 完成后按已保存计划运行重叠章级窗口；每个窗口的术语与子
   checkpoint 原子提交，命中时直接跳过模型。
5. 精确 batch/window 子集合齐全后，在事务内推导 v2 章级父 checkpoint；只有父子全部匹配
   才把 `glossary_status` 更新为 `done`。
...
canonical unit 和窗口并重新推导父 checkpoint。`glossary_status=done` 因而证明最终落盘译文
已经完成这一轮抽取；术语库是跨章累积集合，旧派生项不会按单个 unit 盲删，译法差异仍走
`term_conflicts` 和后续 QA/人工裁决。

每次 prepare 还会审核带 version 1 plan 且显式 `glossary_status=done` 的章节。旧状态核对
batch 与直接章级 v1 checkpoint；带窗口计划的新状态核对精确 batch/window 子集合和派生
章级 v2 父记录。任一记录缺失、额外、世代/计划不符或 target 指纹漂移，就把该章重新标成 pending。术语审计
...
batch 成功事件沿用 `batch_glossary_extracted`；`chapter_glossary_extracted` 只描述兼容的直接
章级 v1 路径。窗口化路径使用独立事件：

- `chapter_glossary_window_extracted`：窗口术语与 `chapter_window` 子 checkpoint 原子提交。
- `chapter_glossary_window_skipped`：窗口子 checkpoint 已匹配，未重复调用模型。
- `chapter_glossary_derived`：精确 batch/window 子集合已在事务内推导并提交 v2 父 checkpoint。

抽取成功事件携带 checkpoint version、完成标记、fingerprint、generation 和 summary。严格
`{"terms":[]}` 得到的全零结果也可信，因为 checkpoint 与空结果在一个事务中提交；只有
没有 checkpoint 字段的 legacy 全零事件不能作为完成证据。事件仍只供审计，v2 完成判定会
直接重查 SQLite 父子记录，而不是信任 JSONL。

其他术语事件：

- `glossary_extraction_failed`：模型或协议失败；含 `phase=batch|chapter|chapter_window`、unit key、
  fingerprint 和 `error_kind`，主翻译可以继续。
- `glossary_persistence_failed`：SQLite 或 checkpoint 提交失败；记录后向上抛出。
- `batch_glossary_skipped`：命中有效 batch checkpoint，未重复调用模型。
- `chapter_glossary_skipped`：全部 batch 与兼容章级 v1 父记录匹配，或 v2 exact children 与
  派生父记录全部匹配，整章无需再执行窗口模型调用。
- `glossary_plan_created`：首次持久化 canonical plan。
- `glossary_plan_invalid`：已保存计划与当前源段不一致；为保护译文而停止续跑。
- `chapter_glossary_plan_created`：首次持久化窗口计划；含计划 fingerprint、window count 和
  source 上限。
- `chapter_glossary_plan_invalid`：窗口计划无法满足约束，或已保存计划与确定性重建结果不同；
  为保护已有译文而停止续跑。
- `chapter_glossary_derivation_deferred`：exact batch/window children 尚未齐全，未写 v2 父记录。
...
- `titles_translated`：标题成功持久化；含 `reference_terms_total`、
  `reference_terms_selected` 和逐项 source/target，证明本次只注入命中标题源文的术语。
